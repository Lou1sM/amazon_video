...
class AtomicFactGenerator(object):
    def __init__(self):
        try:
            ...
            self.nlp = spacy.load("en_core_web_sm")
        self.demo_path = 'PRISMA/demos/demos_complex.json'
        ...
        self.sent_cache_fp = 'PRISMA/sent2facts_cache.json'
...
def text_to_sentences(text):
    sentences = text.removesuffix('- ').split("- ")[1:]
    sentences = [sent.strip()[:-1] if sent!='' and sent.strip()[-1] == '\n' else sent.strip() for sent in sentences]
    if len(sentences) > 0:
        if sentences[-1][-1] != '.':

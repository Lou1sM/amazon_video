...
class OpenAIModel(LM):
    ...
    def _generate(self, prompt, max_output_length=128):
        ...
        This should be about generating a response from the prompt,
        no matter what the application is.
        """

        #if self.model_name == "ChatGPT":
        #message = [{"role": "user", "content": prompt}]
        #response = call_GPT_model(message, model_name="gpt-3.5-turbo-instruct", temp=self.temp, max_len=max_output_length)
        #output = response["choices"][0]["message"]["content"]
        #return output, response
        #elif self.model_name == "InstructGPT":
        response = call_GPT_model(prompt, model_name="gpt-3.5-turbo-instruct", temp=self.temp)
...
#
#def call_GPT_model(message, model_name, max_len=1024, temp=0.7, verbose=False):
#    response = None
#    received = False
#    num_rate_errors = 0
#    while not received:
...
def call_GPT_model(prompt, model_name, max_len=512, temp=0.7, num_log_probs=0, echo=False, verbose=False):

...
def caption_each_scene(ep_name, vl_transformer, tokenizer, tensorizer):
    img_res = 224
    max_num_frames = 64
...
if __name__ == '__main__':
    ...
    tensorizer_ = build_tensorizer(tokenizer_, 50, 1568, 50, is_train=False)

    if ARGS.ep_name == 'all':
        all_ep_names = [fn[:-4] for fn in os.listdir('SummScreen/videos') if fn.endswith('.mp4')]
        for en in all_ep_names:
            caption_each_scene(fn, vl_transformer_, tokenizer_, tensorizer_)
    else:
        caption_each_scene(ARGS.ep_name, vl_transformer_, tokenizer_, tensorizer_)
